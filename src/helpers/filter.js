...
mod.public.byAssignedLimit = function(targets, limit) {
    ...
    var assignees = Game.creeps;
    var result = _.filter(targets, (target) => {
        var id = target.id || target.name; // Assignment ID

        if(mod.public.byAssignmentId(assignees, id).length > limit) { // Filter creeps with assignment ID, if more than limit...
...
// mod.public.byColor(Game.flags, [config.flags.colors.source]);
...
mod.public.byNeedsRepairing = function(targets) {
    ...
    var result = _.filter(targets, (target) => {
        if(_.contains([STRUCTURE_WALL, STRUCTURE_RAMPART], target.structureType)) {
            ...
            return (mod.public.byHits([target], 0, config.structures.walls.minHits - 1000).length > 0);
        } else {
            return (mod.public.byHitsPercentage([target], 0, 80).length > 0); // Could repair to 99 probably without impact
        }
    });
...
mod.public.byNeedsRepairingTopup = function(targets) {
    var result = _.filter(targets, (target) => {
        ...
        if(_.contains([STRUCTURE_WALL, STRUCTURE_RAMPART], target.structureType)) {
            ...
            return (mod.public.byHits([target], config.structures.walls.minHits - 1000, config.structures.walls.minHits).length > 0);
        } else {
            return (mod.public.byHitsPercentage([target], 80, 99).length > 0);
        }
    });
