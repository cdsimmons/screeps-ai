...
global.manage.actions = function() {
	log.cpu('manage.actions', 'start');

	const world = new World();
	var actioners = _.union(world.creeps, world.towers); // Need to combine creeps and structures/towers I think?
	...
	if(actioners.length > 0) { 
		...
		if(destinees.length > 0) {
			for(let destinee of destinees) {
		        ...
		        let target = Game.getObjectById(destination.target.id) || Game.flags[destination.target.id];
		        ...
		        if((target === undefined || target === null) && Game.rooms[assignment.target.pos.roomName] !== undefined) { //) && Game.rooms[assignment.target.pos.roomName] !== undefined
		        ...
		        if(destinee.pos.inRangeTo(targetPos, destinee.getActionRange()) && destination.method !== 'moveTo') {
			        ...
			        if(destinee.transitioned() && Game.flags[destination.id] !== undefined) {
			            delete destinee.memory.destination;
