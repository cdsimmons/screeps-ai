...
global.manage.destinations = function() {
	log.cpu('manage.destinations', 'start');

	const world = new World();
	let assignees = _.union(world.creeps, world.towers);
	...
	if(assignees.length > 0) {
		...
		if(assignees.length > 0) {
			...
			for(let assignee of assignees) {
				...
				// Get the target...
				let target = Game.getObjectById(assignment.target.id) || Game.flags[assignment.target.id];

				// Clearing the assignment if target is doesn't exist...
				if((target === undefined || target === null) && Game.rooms[assignment.target.pos.roomName] !== undefined) {
				...
				if(Game.rooms[assignment.target.pos.roomName] === undefined) {
				...
				if(Game.flags[destination.target.id] !== undefined && destination.type !== undefined) {
					let flag = Game.flags[destination.target.id];
